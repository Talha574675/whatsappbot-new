...
const getData = async (client, id, url,exten,options ) => {
  ...
  const fileName = lastPart.split('?')[0];
      ...
      let extension =''
      const contentType = res.headers['content-type']
       if(contentType.split('/')[1].split('-')[0].length>4){
      ...
      if(extension == 'bin' && !exten) {
       downloadv1(client,id,url,'./files','2nd download option')
       client.sendMessage(id, {text: 'filed to get extension please write extension at the end of link'})
       return
      }
